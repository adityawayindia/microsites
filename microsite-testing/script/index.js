...
// Floating Language Switcher Interactivity
(function () {
  const switcher = document.getElementById("langSwitcher");
  const btn = document.getElementById("langSwitcherBtn");
  const dropdown = document.getElementById("langDropdown");
  const currentLabel = switcher ? switcher.querySelector(".lang-current") : null;
  ...
  options.forEach((opt) => {
    opt.addEventListener("click", (e) => {
      ...
      checkIcon.className = "fa-solid fa-check lang-check";
      checkIcon.setAttribute("aria-hidden", "true");
      opt.appendChild(checkIcon);

      const selectedName = opt.querySelector(".lang-name")?.textContent.split(" ")[0] || "English";
      if (currentLabel) currentLabel.textContent = selectedName;
